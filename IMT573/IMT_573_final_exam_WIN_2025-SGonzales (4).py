...
# Map the categorical data to data that is easier to read
maps = {
    'educ': {
        9: 'grade school',
        12: 'high school',
...
df_affairs = affairs.copy()
df_affairs_map = affairs.copy()
df_affairs['had_affair'] = (df_affairs['affairs'] > 0).astype(int)
df_affairs['log_affairs'] = df_affairs['affairs'].apply(np.log1p)
df_affairs_map['had_affair'] = (df_affairs['affairs'] > 0).astype(int)
df_affairs_map['log_affairs'] = df_affairs['affairs'].apply(np.log1p)
for col_name in maps.keys():
    new_map = {k: process_string(v) for k, v in maps[col_name].items()}
    df_affairs_map[col_name] = df_affairs_map[col_name].map(new_map)

print("DF with Mapped text columns:")
display(df_affairs_map.head())
print("DF with Dummies (Pandas):")
df_affairs_with_dummies = create_dummies(df_affairs_map, ['educ', 'occupation', 'occupation_husb'])
...
logit_model_with_c = smf.logit(logit_formula, data=df_affairs_map).fit()
...
df_affairs_best = df_affairs_map[['affairs', 'had_affair', 'rate_marriage', 'age', 'yrs_married', 
...
best_logit_model_c_fit = smf.logit(new_formula, data=df_affairs_best).fit()
best_logit_model_c_summary = best_logit_model_c_fit.summary()
...
X = sm.add_constant(df_affairs_with_dummies[best_predictors])
y = df_affairs_with_dummies['had_affair']
# Fit the logistic regression model
best_logit_model_dum = sm.Logit(y, X)
best_logit_model_dum_fit = best_logit_model_dum.fit()
best_logit_model_dum_summary = best_logit_model_dum_fit.summary()
print(best_logit_model_dum_summary)
...
# Predict using the Best Model with C() on SIMULATED Data
print("******* TO DO *******"*18)
if False:
    best_logit_model_c_predictions_binary = (best_logit_model_c_fit.predict(df_affairs_means) > 0.5).astype(int)
    print(best_logit_model_c_predictions_binary)
    df_affairs_means_predicted_c = df_affairs_means.copy()
    df_affairs_means_predicted_c['predicted_affair'] = best_logit_model_c_predictions_binary
    bad_pred_dum_c = df_affairs_means_predicted_c[(df_affairs_means_predicted_c['predicted_affair'] != 
                                               df_affairs_means_predicted_c['had_affair'])]
    good_pred_dum_c = df_affairs_means_predicted_c[(df_affairs_means_predicted_c['predicted_affair'] == 
                                               df_affairs_means_predicted_c['had_affair'])]
    display(bad_pred_dum_c)
    print(f"Accuracy of model: {len(good_pred_dum_c)/len(df_affairs_means_predicted_c):.2%}")


# In[21]:


# Predict using the Best Model with Pandas Dummies Simulated Data 
best_logit_model_dum_predictions_binary = (best_logit_model_dum_fit.predict(df_affairs_means) > 0.5).astype(int)
print(best_logit_model_dum_predictions_binary)
df_affairs_means_predicted = df_affairs_means.copy()
df_affairs_means_predicted['predicted_affair'] = best_logit_model_dum_predictions_binary
bad_pred_dum = df_affairs_means_predicted[(df_affairs_means_predicted['predicted_affair'] != 
                                           df_affairs_means_predicted['had_affair'])]
good_pred_dum = df_affairs_means_predicted[(df_affairs_means_predicted['predicted_affair'] == 
                                           df_affairs_means_predicted['had_affair'])]
display(bad_pred_dum)
print(f"Accuracy of model: {len(good_pred_dum)/len(df_affairs_means_predicted):.2%}")
...
for i in range(1, len(predictors) + 1):
    print(f"Trying {predictors[:i]} with OLS using C() for Categorical variables")
    selected_predictors = predictors[:i]

    # Construct the formula string
    formula_parts = []
    for predictor in selected_predictors:
        if predictor in ['educ', 'occupation', 'occupation_husb']:
            formula_parts.append(f"C({predictor})")
        else:
            formula_parts.append(predictor)

    formula = "affairs ~ " + " + ".join(formula_parts)

    # Fit OLS model using smf.ols
    model = smf.ols(formula, data=affairs)
    result = model.fit(maxiter=1000)

    # Get AIC and Adjusted R-squared
    aic = result.aic
    adj_r_squared = result.rsquared_adj
    ...
    # Get p-values
    p_values = result.pvalues.round(8)
    good_p_values = p_values[p_values <= 0.05]
    # print('P-Values:')
    # print(good_p_values.to_string())
    print('='*50)
...
# First we tried Stepwise Selection with numerical values in the categorical variables and then we used the C() formulas to let Statsmodel do it automatically. The result was the same:<p>
...
print(f"Best Fit OLS Using C() for Categorical Variables {predictors}")
formula_parts = []
...
# <b>Answer:</b><p>
# Based on my Python code in cell `55` above, the lower probability is far more likely, as I never even got one hit. I would chose Alternate 1 or 2.
